    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          case 'list_tables': {
            const tables = await this.datalogClient.listTables(args?.project_id as string);
            return {
          ...
          case 'list_columns': {
            const columns = await this.datalogClient.listColumns(
          ...
          case 'list_assets': {
            const assets = await this.datalogClient.listAssets(
              args?.project_name as string,
              args?.collection_name as string,
            );
            return {
          ...
          case 'upload_text': {
            const result = await this.datalogClient.uploadPlainText(
              args?.project_name as string,
              args?.collection_name as string,
              args?.text as string,
