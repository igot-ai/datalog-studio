      ...
      return {
        tools: [
          ...
          },
          {
            name: 'create_record',
            description:
              'Insert a new record (row) into a collection. ' +
            ...
            inputSchema: {
              ...
              properties: {
                catalog_name: {
                  type: 'string',
                  description: 'Name of the catalog (project)',
                },
                collection_name: {
                  type: 'string',
                ...
                column_values: {
                  type: 'object',
                  description:
                    'Column values to set at creation time. ' +
                    'Keys are column names (call list_attributes to discover them); values are the data for each column. ' +
                    'For columns you want left empty / AI-extracted, pass an empty object {}. ' +
                    'Example: { "id": "user_001", "name": "Alice", "segment_asia": {} }',
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          case 'list_members': {
            const members = await this.dataClient.listProjectMembers(args?.project_id as string);
            return {
          ...
          case 'assign_member': {
            const member = await this.dataClient.assignMember(args?.project_id as string, {
          ...
          case 'update_member_role': {
            const result = await this.dataClient.updateMemberRole(
          ...
          case 'invite_member': {
            const invitation = await this.dataClient.createInvitation(args?.project_id as string, {
          ...
          case 'create_record': {
            const result = await this.dataClient.createRecord(
              args?.catalog_name as string,
              args?.collection_name as string,
              {
                plainText: args?.plain_text as string | undefined,
                sourceId: args?.source_id as string | undefined,
                columnValues: args?.column_values as Record<string, any> | undefined,
                transform: args?.transform as boolean | undefined,
              },
            );
            return {
          ...
          case 'list_physical_tables': {
            const tables = await this.dataClient.listPhysicalTables(args?.project_id as string);
            return {
          ...
          case 'describe_physical_table': {
            const schema = await this.dataClient.describePhysicalTable(args?.table_id as string);
            return {
              content: [{ type: 'text', text: JSON.stringify(schema, null, 2) }],
            };
          }
